...
export async function POST(req: NextRequest) {
    ...
    try{
        ...
        return NextResponse.json({ message: 'Cour crée', course: newCourse}, { status: 201 });
    }catch(e){
        console.log('error', e)
        return NextResponse.json({ error: 'Connection error' }, { status: 500 });
    }
...
export async function PATCH(req: NextRequest) {
    ...
    try{
        ...
        return NextResponse.json({ message: 'Cour mise à jour', course: JSON.stringify(updateCourse) });
    }catch{
        return NextResponse.json({ error: 'Connection error' }, { status: 500 });
    }
}